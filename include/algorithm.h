...
namespace Algorithm
{
    ...
    // Battle（Boss战算法接口）输入形式 const GameState& state, const BossMemory& bossMemory 输入含义 当前Boss战可见状态、AI已知Boss记忆 输出形式 Action 输出含义 技能动作或NONE动作
    Action Battle(const GameState& state, const BossMemory& bossMemory);
}
